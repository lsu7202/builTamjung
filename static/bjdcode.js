...
/**
 * BJD_MAP은 파이썬 스크립트로 추출한 { "코드": "전체주소" } 형태의 객체입니다.
 * (예: "1168010100": "서울특별시 강남구 역삼동")
 */

// 지번(본번-부번)을 8자리 규격으로 변환하는 헬퍼 함수
function formatBunji(bunjiStr) {
    if (!bunjiStr) return "";
    const parts = bunjiStr.split('-');
    const main = parts[0].padStart(4, '0'); // 본번 4자리
    const sub = (parts.length > 1 ? parts[1] : "0").padStart(4, '0'); // 부번 4자리 (없으면 0000)
    return main + sub;
}
...
export function convertAddressToPNU(input) {
    ...
    addrInputs.forEach(rawAddr => {
        const bunjiMatch = rawAddr.match(/\d+(-\d+)?$/);
        const bunji = bunjiMatch ? bunjiMatch[0] : null;
        const regionKeyword = bunji ? rawAddr.replace(bunji, "").trim() : rawAddr.trim();

        // BJD_MAP에서 법정동 코드 검색
        const matchedCodes = Object.keys(BJD_MAP).filter(code => {
            const fullAddr = BJD_MAP[code];
            return fullAddr.includes(regionKeyword) || regionKeyword.includes(fullAddr);
        });

        matchedCodes.forEach(code => {
            if (bunji) {
                const formattedBunji = formatBunji(bunji);
                // 0(대지)과 1(산)을 모두 리스트에 추가하여 DB가 IN 연산으로 찾게 함
                totalPnuResults.push(code + "0" + formattedBunji);
                totalPnuResults.push(code + "1" + formattedBunji);
            } else {
                // 지번 없으면 동 단위 검색용 10자리 리턴
                totalPnuResults.push(code);
            }
        });
    });

    return [...new Set(totalPnuResults)];
}